        ...
        if (resultCode == Activity.RESULT_OK) {
            when (requestCode) {
                ...
                REQUEST_IMAGE_CAPTURE -> {
                    ...
                    if (imageUri!= null) {
                        val bitmap = getCorrectlyOrientedImage(imageUri!!)
                        imageView.setImageBitmap(bitmap)
                        closeButton.visibility = View.VISIBLE
                    }
                    else {
                        Toast.makeText(this, "Ошибка получения изображения", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
    ...
    private fun applyDetecting(imageUri: Uri, drawFrames: Boolean) {
        launch {
            val bitmap = withContext(Dispatchers.IO) {
                getCorrectlyOrientedImage(imageUri)
            }
